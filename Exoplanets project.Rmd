...
# Read in data and drop useless first column
ep <- read.csv('planets_clean.csv', sep = ',')
ep <- ep[, 2:dim(ep)[2]]
getwd()
```
...
hist_plot(trans, trans$pl_bmassj, .1) + xlab('Jupiter Masses')
...
hist_plot(rv, rv$pl_bmassj, .1) + xlab('Jupiter Masses')
...
hist_plot(im, im$pl_bmassj, .1) + xlab('Jupiter Masses')
...
```{r echo=FALSE, Su}
summary(num_planets)
ggplot(num_planets, aes(x = pl_letter)) + geom_bar()
