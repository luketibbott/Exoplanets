...
ggplot(ep, aes(x = st_teff)) + geom_histogram(binwidth = 100) +
  xlab('Effective temperature (K)') + coord_cartesian(xlim = c(0, 10000))
...
according to Wikipedia). The second plot clearly shows two different groups of
planets.
...
ggplot(ep, aes(x = pl_orbsmax, y = pl_bmassj)) + geom_point() +
...
ggplot(ep, aes(x = st_mass, y = pl_bmassj)) + geom_point(alpha=0.5) +
...
temperatures among stars that have exoplanets orbiting them.
...
lm(pl_radj ~ st_teff, data = ep) %>% summary()
...
> Ther appears to be some non-linear relationship between a star's distance from
earth and its optical magnitude. I'd guess this is a quadratic relationship 
because light propagates according to the inverse *square* law.
...
ggpairs(ep[ , -which(names(ep) %in% c('pl_hostname', 'pl_name'))])
```
...
> The relationship between planetary mass and planetary radius is strong and positive. I believe this is the strongest relationship in my bivariate analysis.
...
m = subset(ep, pl_discmethod == 'Transit' | pl_discmethod == 'Radial Velocity' | 
           pl_discmethod == 'Microlensing' | pl_discmethod == 'Imaging')

ggplot(m, aes(x = st_dist, y = pl_bmassj, color = pl_discmethod)) +
  geom_point(alpha = 0.2) + scale_color_brewer(type='qual', palette = 2) + 
...
ggplot(ep, aes(x = pl_orbsmax, y = pl_bmassj, color = pl_discmethod)) + geom_point() + 
